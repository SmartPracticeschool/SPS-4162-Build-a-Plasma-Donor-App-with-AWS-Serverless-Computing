...
def check(email):
    url = "https://ckinvsqtdf.execute-api.us-east-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://ckinvsqtdf.execute-api.us-east-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://ckinvsqtdf.execute-api.us-east-1.amazonaws.com/plasma/bloodgroupcount"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://ckinvsqtdf.execute-api.us-east-1.amazonaws.com/plasma/requestblooduser?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=i2LvC3XAD0Vgw4IulkF6opeJSNTjdYHrsZOqz7nUQa1McGh5fxDYiI3POrzZ1J978SqsKRumf0MvQjB6&sender_id=FSTSMS&message=Urgent! Need Plasma same as your Blood Group&language=english&route=p&numbers="+str(i['phone'])
